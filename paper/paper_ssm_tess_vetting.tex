...
The Transiting Exoplanet Survey Satellite (TESS)~\cite{ricker2015} observes
$>$200{,}000 stars at
...
TOIs from the full global light curve. Concurrent work on Kepler transit detection
identifies the same opening independently: ExoVeil~\cite{exoveil} notes that a
linear-complexity backbone such as Mamba would allow a complete 65{,}000-point
light curve to be processed in a single pass, but records this as future work
rather than evaluating it. That gap is the niche this study addresses with a
controlled, low-cost experimental comparison.

% =============================================================================
\section{Data and Problem Setup}

\textbf{Sources.} Labels come from the NASA Exoplanet Archive TOI
catalog~\cite{toi,guerrero2021}; light curves are the PDCSAP\_FLUX series
retrieved from MAST~\cite{mast} via \texttt{lightkurve}~\cite{lightkurve}. We define a binary
...
per-sample test predictions. The sealed test set is guarded programmatically: the
evaluation CLI defaults to the validation split, and evaluating the test split
appends a record (timestamp, git commit, sample count, AUC) to a versioned ledger
and then refuses to run again for any run directory already listed there unless an
explicit override flag is passed---an override that is itself written to the
ledger. A suite of 73 unit tests covers the data and metrics paths and asserts the
split guarantees directly: no TIC is shared across train/validation/test, class
balance is preserved under stratification, and the Tier-2 subsets are nested
inside their Tier-1 parents. Inference statistics are computed from the stored
predictions by a single script, so all confidence intervals and DeLong tests in
this paper are reproducible without retraining.
...
\bibitem{exoveil} P.~Priyanshu, ``One Transit Is All You Need: Detecting
Exoplanets Through Learned Stellar Behaviour with ExoVeil,'' arXiv:2606.02778,
2026.
...
\bibitem{ricker2015} G.~R.~Ricker \emph{et al.}, ``Transiting Exoplanet Survey
Satellite (TESS),'' \emph{J. Astron. Telesc. Instrum. Syst.}, vol.~1, no.~1,
p.~014003, 2015.

\bibitem{guerrero2021} N.~M.~Guerrero \emph{et al.}, ``The TESS Objects of
Interest Catalog from the TESS Prime Mission,'' \emph{Astrophys. J. Suppl. Ser.},
vol.~254, no.~2, p.~39, 2021.
