...
(\textit{sanity check}): \textbf{¿puede el modelo memorizar un
subconjunto chico de datos?}

El procedimiento es directo: se selecciona un subconjunto de 64
ejemplos del \textit{train}, se apaga la regularización
(\textit{dropout} = 0, \textit{weight decay} = 0) y se entrena el
modelo hasta que su AUC sobre el subconjunto llegue a $1.0$. Un modelo
sano debería lograrlo en pocas decenas de épocas. Si no lo logra, no
es problema de datos: es señal de que la arquitectura o el
\textit{pipeline} está roto (gradientes que no fluyen, normalización
mal puesta, error en el \textit{loader}, \textit{etc.}), y conviene
arreglarlo antes de invertir cómputo en entrenamientos largos.

\begin{table}[H]
\centering
\caption{Resultados de \textit{sanity overfit} (subset $N=64$, sin
\textit{dropout}). Ambos modelos memorizan el subconjunto, lo que
descarta errores estructurales antes de los entrenamientos reales.}
\label{tab:sanity}
\begin{tabular}{lcc}
\toprule
Modelo & Epoch hasta val\_auc $=1.0$ & Resultado \\
\midrule
CNN \textit{single-branch} (Sec.\ref{subsec:cnn})   & TODO & \checkmark \\
Mamba \textit{single-view} (Sec.\ref{subsec:mamba}) & TODO & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

Este chequeo es una pequeña inversión que ahorra mucho debugging
después. Su valor metodológico es que separa los problemas de
arquitectura (que se ven aquí) de los problemas de aprendizaje
(que se ven en el entrenamiento real con el \textit{dataset} completo).
...
\midrule
\texttt{mamba\_small\_seed42}    & TODO \\
\texttt{mamba\_small\_seed123}   & TODO \\
\texttt{mamba\_small\_seed456}   & TODO \\
\texttt{mamba\_small\_seed789}   & TODO \\
\texttt{mamba\_small\_seed2024}  & TODO \\
\midrule
\textbf{Media $\pm$ desv.\ estándar} & \textbf{TODO} \\
\bottomrule
\end{tabular}
\end{table}
